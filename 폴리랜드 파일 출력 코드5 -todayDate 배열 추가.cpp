...
int main()
{
	...
	do{ //다음 손님 받기 위한 do while 반복  
		int ticketArr[20] = {0}, ticketTimeArr[20] = {0}, treatmentArr[20] = {0}, ticketNumberArr[20] = {0},
			priceArr[20] = {0}, totalPriceArr[20] = {0} , ageGroupArr[20] = {0} , treatmentGroupArr[20]= {0}, todayDate[20]= {0};
		...
		//	if(fscanf(filePointer, "%c", &temp) == -1){
			...
			for (int i =0; i<counter; i++){				
				...
				fprintf(filePointer, "날짜, 권종, 권종시간, 연령, 수량, 가격, 우대사항\n");////////////////////////////////////////////////////////////오류 처리  필요  
							fprintf(filePointer,"%d,%d,%d,%d,%d,%d,%d\n",
								todayDate[i],ticketArr[i],ticketTimeArr[i],ageGroupArr[i],ticketNumberArr[i],totalPriceArr[i],treatmentGroupArr[i]);	
			}
				switch (ticketArr[i]) {
				case 1:
					printf("\t종합이용권");
